...
The page is a masthead-over-plate composition, unchanged by the recolor. On `lg+` screens it locks into a two-pane grid, `minmax(0,1fr)` for the map pane against a fixed `minmax(360px,420px)` registry column, with only the column scrolling (`lg:overflow-y-auto`) while the map pane fills the remaining height. Below `lg` it stacks: masthead, filter strip, a `62svh` map (min 340px), the time rule, then the registry column in normal document flow.

Structure, top to bottom: a double-rule masthead (now on a `sello` field: nameplate + data-cut metadata + section tabs + the "Reportar" notice), a horizontally-scrollable filter strip (province/canton selects, incident-type toggles, detentions toggle), the map plate with two floating overlays (the legend, bottom-left; the "Mi ubicación" control and transient notices, top-right/top-left), the time rule (year ribbon over a month ribbon), and finally the registry column itself.
...
### Masthead
Double-rule header (`.double-rule`) whose top field is now `bg-sello`/`text-paper` (previously plain paper): the nameplate, a right-aligned tabular metadata list (Corte de datos / Fuente, both in `text-paper/85`), sit on the stamp-blue field. Below it, on a plain paper field, a tab bar (Registro / Metodología / Fuentes) inverts its active tab to `bg-sello`/`text-paper`. The "Reportar un incidente" entry point is hatched (`.hatch`) at rest and inverts to `bg-sello`/`text-paper` once opened, revealing an `.ink-in`-animated popover that states plainly it arrives in a future version.

### Filter Strip
A horizontally-scrollable row of bordered controls: `PlaceSelect` (a bordered field with a label chip, a native `<select>`, an inline SVG chevron — unchanged, still neutral ink/sheet), incident-type pill toggles (bordered rectangle with a `Mark` icon in its type hue and a tabular count, inverting to `bg-sello`/`text-paper` when active — previously `bg-ink`), and the detentions toggle, the strip's one dashed-border control, turning solid and `bg-sello`/`text-paper` only once switched on.
